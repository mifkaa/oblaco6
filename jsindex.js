...
// blockingPlay.addEventListener('click', () => {
//   gameContainerBox.style.display = 'none';
//   body.style.overflowY = 'auto';
//   // blockingPlay.style.display = 'none';
...
// // document.querySelectorAll('#pageMain .grid > div').forEach(item => {
// //   // Сохраняем оригинальную opacity
// //   const originalOpacity = window.getComputedStyle(item).opacity;
  
// //   // Устанавливаем начальное состояние только для анимации появления
// //   item.style.opacity = '0';
...
// //     entries.forEach(entry => {
// //       if (entry.isIntersecting) {
// //         // Возвращаем оригинальную opacity (0.5 или любое другое значение)
// //         entry.target.style.opacity = originalOpacity;
        
// //         setTimeout(() => {
// //           // Убираем transition чтобы не мешать дальнейшим изменениям
// //           entry.target.style.transition = '';
// //         }, 700);
...
// // document.querySelectorAll('#pageMain .grid > div').forEach(item => {
// //   // Сохраняем оригинальные стили в data-атрибутах
// //   const originalTransform = window.getComputedStyle(item).transform;
// //   const originalOpacity = window.getComputedStyle(item).opacity;
  
// //   item.dataset.originalTransform = originalTransform;
// //   item.dataset.originalOpacity = originalOpacity;

// //   // Устанавливаем начальное состояние для анимации
// //   item.style.transform = originalTransform === 'none' 
// //     ? 'translateY(30px)' 
// //     : `${originalTransform} translateY(30px)`;
